...
import com.jan.betaplat.core.util.StringUtil;
...
public class IPSeeker{
		...
		for (long offset = ipBegin + 4; offset <= endOffset; offset += IP_RECORD_LENGTH) {
			...
			if (temp != -1) {
				...
				if (loc.country.indexOf(s) != -1 || loc.area.indexOf(s) != -1) {
					IPModel entry = new IPModel();
					entry.country = loc.country;
					entry.area = loc.area;
					// 得到起始IP
					readIP(offset - 4, b4);
					entry.beginIp = LocationUtil.getIpStringFromBytes(b4);
					// 得到结束IP
					readIP(temp, b4);
					entry.endIp = LocationUtil.getIpStringFromBytes(b4);
					// 添加该记录
					ret.add(entry);
				}
			}
	...
	*/
	public String getCountry(byte[] ip) {
		...
		if (ipFile == null)
			return "错误的IP数据库文件";
		// 保存ip，转换ip字节数组为字符串形式
		String ipStr = LocationUtil.getIpStringFromBytes(ip);
	...
	*/
	public String getCountry(String ip) {
		return getCountry(LocationUtil.getIpByteArrayFromString(ip));
	}
	...
	*/
	public String getArea(String ip) {
		return getArea(LocationUtil.getIpByteArrayFromString(ip));
	}
